...
from apps.mainapp.models import Report, City, Ward
...
class LatestReports(Feed):
    ...
    def items(self):
        return Report.active.order_by('-created_at')[:30]
...
class LatestReportsByCity(Feed):
    ...
    def items(self, obj):
       return Report.active.filter(ward__city=obj.id).order_by('-created_at')[:30]
...
class LatestReportsByWard(Feed):
    ...
    def items(self, obj):
       return Report.active.filter(ward=obj.id).order_by('-created_at')[:30]
